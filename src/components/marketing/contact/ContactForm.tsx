...
export default function ContactForm({ lang, dict, cardsSlot }: ContactFormProps) {
  ...
  const formBody = (
    <form className="contact-form contact-form-container w-full h-full flex flex-col justify-between" onSubmit={handleSubmit} noValidate>
      <div className="flex flex-col gap-4">
        {/* Row 1: Name & Email */}
        <div className="contact-form-row grid grid-cols-1 gap-4 sm:grid-cols-2">
          {/* Your Name */}
          <div className="contact-field-group">
            <label htmlFor="contact-name" className="contact-label text-left rtl:text-right text-slate-800 dark:text-zinc-200 font-medium text-sm flex items-center gap-1">
              <span>{dict?.nameLabel || (isRtl ? "اسمك" : "Your name")}</span>
              <span className="contact-required text-rose-500 dark:text-rose-400" aria-hidden="true">*</span>
            </label>
            <input
              id="contact-name"
              name="name"
          ...
          {/* Your Email */}
          <div className="contact-field-group">
            <label htmlFor="contact-email" className="contact-label text-left rtl:text-right text-slate-800 dark:text-zinc-200 font-medium text-sm flex items-center gap-1">
        ...
        <div className="contact-field-group">
          ...
          >
            ...
            <input
              ...
              className="contact-phone-input placeholder:text-slate-400 dark:placeholder:text-zinc-500 text-slate-900 dark:text-zinc-100 outline-none text-left w-full bg-transparent px-3 text-sm"
            />
          </div>
        </div>
        ...
        <div className="contact-field-group">
          ...
          <div className="contact-textarea-wrapper relative w-full">
            <textarea
              ...
              onChange={handleInputChange}
              placeholder={dict?.messagePlaceholder || (isRtl ? "اكتب رسالتك هنا..." : "Type here...")}
              className="contact-textarea h-32 lg:h-36 rounded-xl border border-slate-300 dark:border-white/[0.09] bg-white/80 dark:bg-zinc-900/30 backdrop-blur-md dark:shadow-[inset_0_1px_1px_rgba(255,255,255,0.03)] placeholder:text-slate-400 dark:placeholder:text-zinc-500 text-slate-900 dark:text-zinc-100 focus:border-slate-400 focus:dark:border-white/30 focus:dark:ring-1 focus:dark:ring-white/20 outline-none w-full transition-all resize-none p-3.5 pb-8 text-sm"
            />
            <span className="contact-char-count absolute bottom-2.5 right-3.5 rtl:right-auto rtl:left-3.5 text-slate-500 dark:text-zinc-400 text-xs font-mono select-none" aria-live="polite">
      ...
      {/* Submit Action: Clearly visible, properly padded, aligned */}
      <div className="contact-form-actions flex flex-col items-stretch lg:items-end rtl:lg:items-start mt-4 pt-1">
        ...
        <button
          ...
          className="contact-btn-submit w-full sm:w-auto inline-flex items-center justify-center gap-2 py-3 px-8 rounded-xl text-sm font-semibold bg-slate-950 dark:bg-white text-white dark:text-slate-950 hover:bg-slate-800 dark:hover:bg-slate-100 transition-all shadow-sm active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed cursor-pointer"
        >
          <span>{isSubmitting ? (isRtl ? "جاري الإرسال..." : "Sending...") : dict?.submitButton || (isRtl ? "إرسال" : "Submit")}</span>
          {isRtl ? (
